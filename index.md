---
title: Your Name — Tech, Teaching & Careers
description: 
---

# 👋 Hi, I’m <Your Name>

Brief intro (1–2 lines): what you do, who you help, and how.  
*Example:* “”

---

## 🧭 Quick Navigation
- [Courses](#-courses)
- [Talks & Resources](#-talks--resources)
- [☕ Coffee Shop (Career Hub)](#-coffee-shop-career-hub)
- [Live Tools & Demos](#-live-tools--demos)
- [Events / Office Hours](#-events--office-hours)
- [Contact / Connect](#-contact--connect)
- [FAQ](#-faq)

---

## 🎓 Courses
**Python for AI (YouTube series)**  


---

## 🎤 Talks & Resources
**Canadian IT Workplace Integration (Guest Lecture)**  
- 📑 Slides (PDF): _link_  
- 📦 Resource Hub (tools/checklists): _link_  
- 🧭 Newcomer Guide (resume, email tone, meetings): _link_


---

## ☕ Coffee Shop (Career Hub)
*A cozy corner for role-based readiness with interactive checklists.*  
Use the checklists, analyze your skill gaps, and save a 1-page prep sheet before interviews.

- 🎨 Designers → **[Open Checklist](coffee-shop.html?role=designer)**
- 💻 Developers → **[Open Checklist](coffee-shop.html?role=developer)**
- 🧪 Testers / QA → **[Open Checklist](coffee-shop.html?role=tester)**
- 🧭 Business Systems Analysts → **[Open Checklist](coffee-shop.html?role=analyst)**

Explore skills ↔ careers: [Interactive Map](skills-careers.html)
> Your progress is saved **in your browser** (no sign-in).  
> Use **Analyze My Skill Gaps** to get tailored next steps.

---

## 🛠️ Live Tools & Demos
Practical, free tools you can use right now.

- 🧾 **STAR Story Builder** → _link_  
- 🔍 **Resume Keyword Matcher** (resume vs job post) → _link_  
- ✉️ **Email Tone Polisher** (Canadian-friendly) → _link_  
- 🧪 **Interview Practice Timer** (60/90/120s) → _link_  
- 🗂️ **Application Tracker (Local)** → _link_

> Want to contribute a tool? Open a PR or submit an issue with a short description.


---

## 🤝 Contact / Connect
- 💼 LinkedIn: _link_  
- 📨 Contact form (preferred): _Google Form or Formspree link_  
- 🐙 GitHub: _link_

> Include “**Coffee Shop**” or the event name in your message so I don’t miss it.

---


## 🧾 Site Notes
- Built with **GitHub Pages** (static, no tracking).  
- Checklists save to your **browser (localStorage)** only.  


